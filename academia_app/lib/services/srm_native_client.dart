...
class SrmNativeClient {
  ...
  Future<Map<String, String>> _getSigninSessionHeaders() async {
    final url = Uri.parse('https://academia.srmist.edu.in/accounts/p/40-10002227248/signin?hide_fp=true&servicename=ZohoCreator&service_language=en&dcc=true&serviceurl=https%3A%2F%2Facademia.srmist.edu.in%2Fportal%2Facademia-academic-services%2FredirectFromLogin');
    ...
    if (kDebugMode) debugPrint("[SRM AUTH] Step 1 redirect: ${(res.statusCode >= 300 && res.statusCode < 400)}");

    final setCookie = res.headers['set-cookie'];
    if (kDebugMode) debugPrint("[SRM AUTH] RAW Set-Cookie from Step 1: $setCookie");
    ...
    for (final pair in cookie.split(';')) {
      ...
      if (csrfToken == null && p.startsWith('_zcsr_tmp=')) csrfToken = p.substring('_zcsr_tmp='.length);
    }
    
    if (kDebugMode) debugPrint("[SRM AUTH] CSRF/session seed detected: ${csrfToken != null && csrfToken.isNotEmpty}");
  ...
  Future<Map<String, dynamic>> _validateUser(String username) async {
    ...
    final url = Uri.parse('https://academia.srmist.edu.in/accounts/p/40-10002227248/signin/v2/lookup/${Uri.encodeComponent(username)}');
    final req = http.Request('POST', url)..followRedirects = false;
    req.headers['accept'] = '*/*';
    req.headers['content-type'] = 'application/x-www-form-urlencoded;charset=UTF-8';
    if (csrfToken.isNotEmpty) req.headers['x-zcsrf-token'] = 'iamcsrcoo=$csrfToken';
    if (cookie.isNotEmpty) req.headers['cookie'] = cookie;
    req.headers['Referer'] = 'https://academia.srmist.edu.in/accounts/p/40-10002227248/signin?hide_fp=true&servicename=ZohoCreator&service_language=en&dcc=true&serviceurl=https%3A%2F%2Facademia.srmist.edu.in%2Fportal%2Facademia-academic-services%2FredirectFromLogin';
    req.body = 'mode=primary&cli_time=${DateTime.now().millisecondsSinceEpoch}&servicename=ZohoCreator&service_language=en&serviceurl=https%3A%2F%2Facademia.srmist.edu.in%2Fportal%2Facademia-academic-services%2FredirectFromLogin';
    ...
    if (kDebugMode) {
       ...
       debugPrint("[SRM AUTH] Step 2 Content-Type: ${res.headers['content-type']}");
       debugPrint("[SRM AUTH] Step 2 Content-Length: ${res.headers['content-length']}");
    ...
    if (res.statusCode == 200 || res.statusCode == 201) {
      ...
      if (kDebugMode) debugPrint("[SRM AUTH] Identifier received: $hasIdentifier");
    ...
    if (kDebugMode) debugPrint("[SRM AUTH] Step 2 FAILURE TYPE: HTTP ${res.statusCode}");
    return {'success': false, 'error': 'User lookup failed: ${res.statusCode}'};
  }

  Future<bool> _terminateSessions(String sessionCookie, String csrfToken) async {
    if (kDebugMode) debugPrint("[SRM AUTH] Attempting to terminate sessions (435 recovery)");
    final url = Uri.parse('https://academia.srmist.edu.in/accounts/p/40-10002227248/webclient/v1/announcement/pre/blocksessions');
    final req = http.Request('DELETE', url)..followRedirects = false;
    req.headers['accept'] = 'application/json, text/javascript, */*; q=0.01';
    req.headers['content-type'] = 'application/x-www-form-urlencoded;charset=UTF-8';
    if (csrfToken.isNotEmpty) req.headers['X-ZCSRF-TOKEN'] = 'iamcsrcoo=${Uri.encodeComponent(csrfToken)}';
    if (sessionCookie.isNotEmpty) req.headers['cookie'] = sessionCookie;
    req.headers['Referer'] = 'https://academia.srmist.edu.in/accounts/p/40-10002227248/preannouncement/block-sessions';
    final res = await _client.send(req);
    await res.stream.drain();
    if (kDebugMode) debugPrint("[SRM AUTH] Session termination status: ${res.statusCode}");
    return (res.statusCode >= 200 && res.statusCode < 300);
  }

  Future<Map<String, dynamic>> _validatePassword(String identifier, String digest, String password, String cookie) async {
    String csrfToken = '';
    for (final pair in cookie.split(';')) {
      final p = pair.trim();
      if (p.startsWith('iamcsr=')) csrfToken = p.substring('iamcsr='.length);
      if (csrfToken.isEmpty && p.startsWith('_zcsr_tmp=')) csrfToken = p.substring('_zcsr_tmp='.length);
    }
    
    if (kDebugMode) debugPrint("[SRM AUTH] Step 3 request started");

    final url = Uri.parse('https://academia.srmist.edu.in/accounts/p/40-10002227248/signin/v2/primary/$identifier/password?digest=$digest&cli_time=${DateTime.now().millisecondsSinceEpoch}&servicename=ZohoCreator&service_language=en&serviceurl=https%3A%2F%2Facademia.srmist.edu.in%2Fportal%2Facademia-academic-services%2FredirectFromLogin');
    final req = http.Request('POST', url)..followRedirects = false;
    req.headers['accept'] = '*/*';
    req.headers['content-type'] = 'application/x-www-form-urlencoded;charset=UTF-8';
    if (csrfToken.isNotEmpty) req.headers['x-zcsrf-token'] = 'iamcsrcoo=$csrfToken';
    if (cookie.isNotEmpty) req.headers['cookie'] = cookie;
    req.headers['Referer'] = 'https://academia.srmist.edu.in/accounts/p/40-10002227248/signin?hide_fp=true&servicename=ZohoCreator&service_language=en&dcc=true&serviceurl=https%3A%2F%2Facademia.srmist.edu.in%2Fportal%2Facademia-academic-services%2FredirectFromLogin';
    req.body = jsonEncode({"passwordauth": {"password": password}});
    ...
    if (kDebugMode) {
       ...
       debugPrint("[SRM AUTH] Step 3 Content-Type: ${res.headers['content-type']}");
       debugPrint("[SRM AUTH] Step 3 Content-Length: ${res.headers['content-length']}");
    ...
    if ((res.statusCode == 301 || res.statusCode == 302 || res.statusCode == 303 || res.statusCode == 307 || res.statusCode == 308) && location.contains('sessions-reminder')) {
      if (kDebugMode) debugPrint("[SRM AUTH] Step 3 redirect detected (435 session reminder)");
      return {'success': false, 'status': 435, 'cookie': newCookie, 'csrfToken': csrfToken};
    }
    ...
    if (res.statusCode == 200 || res.statusCode == 201) {
      try {
          ...
          if (data['status_code'] == 435 || data['status_code'] == '435') {
            if (kDebugMode) debugPrint("[SRM AUTH] Step 3 FAILURE TYPE: HTTP 435 (sessions full)");
            return {'success': false, 'status': 435, 'cookie': newCookie, 'csrfToken': csrfToken};
          }
          if (kDebugMode) debugPrint("[SRM AUTH] Authentication success detected: true");
          return {'success': true, 'cookie': newCookie};
      } catch(e) {
          if (kDebugMode) debugPrint("[SRM AUTH] Step 3 FAILURE TYPE: parser/JSON error");
      }
    }

    if (kDebugMode) debugPrint("[SRM AUTH] Step 3 FAILURE TYPE: HTTP ${res.statusCode} ${res.headers['location'] ?? ''}");
    if (kDebugMode) debugPrint("[SRM AUTH] Authentication success detected: false");
    return {'success': false, 'error': 'Password validation failed: ${res.statusCode}'};
  }
  ...
  Future<bool> login(String email, String password) async {
    try {
      ...
      var passRes = await _validatePassword(identifier, digest, password, cookie);
      if (passRes['success'] == false && passRes['status'] == 435) {
        final termRes = await _terminateSessions(passRes['cookie'], passRes['csrfToken']);
        if (termRes) {
          if (kDebugMode) debugPrint("[SRM AUTH] Retrying after session termination (restarting flow)");
          final retryUserRes = await _validateUser(email);
          if (retryUserRes['success'] != true) return false;
          passRes = await _validatePassword(retryUserRes['identifier'], retryUserRes['digest'], password, retryUserRes['cookie']);
        } else {
      ...
      if (kDebugMode) debugPrint("[SRM AUTH] Native login failed (Exception): $e");
      return false;
    }
  }

  String? _decodePageSanitizer(String response) {
    final match = RegExp(r"pageSanitizer\.sanitize\s*\(\s*'(.*?)'\s*\)", dotAll: true).firstMatch(response);
    if (match == null || match.groupCount < 1) return null;
    String decoded = match.group(1)!;
    decoded = decoded.replaceAllMapped(RegExp(r'\\x([0-9A-Fa-f]{2})'), (m) => String.fromCharCode(int.parse(m.group(1)!, radix: 16)));
  ...
  Future<Map<String, dynamic>> fetchAllData() async {
    final rawHtml = await _fetchPage('https://academia.srmist.edu.in/srm_university/academia-academic-services/page/My_Attendance');
    ...
    final regMatch = RegExp(r'Registration Number\s*:\s*([A-Za-z0-9]+)', caseSensitive: false).firstMatch(text);
    final nameMatch = RegExp(r'Name\s*:\s*([a-zA-Z\s]+?)(?:Program|Department|$)', caseSensitive: false).firstMatch(text);
    final progMatch = RegExp(r'Program\s*:\s*([a-zA-Z\.\s]+?)(?:Department|$)', caseSensitive: false).firstMatch(text);
    final semMatch = RegExp(r'Semester\s*:\s*(\d+)', caseSensitive: false).firstMatch(text);
    ...
    for (final table in tables) {
      ...
      if (headerText.contains('course code') && headerText.contains('conducted') && headerText.contains('absent')) {
        attTable = table;
        break;
      }
    }
    ...
    if (attTable != null) {
      ...
      for (final row in rows) {
        ...
        if (cols.length >= 8) {
          ...
          coursesMap[code] = {
            ...
            'hours_absent': absent,
            'attendance_percentage': double.parse(percentage.toStringAsFixed(2)),
          };
      ...
      if (totalConducted > 0) {
        overallAttendance = double.parse((((totalConducted - totalAbsent) / totalConducted) * 100).toStringAsFixed(2));
      }
    }
    ...
    if (marksTable != null) {
      ...
      for (final row in rows) {
        ...
        if (cols.length >= 3) {
          ...
          if (course.isEmpty || category.isEmpty || innerTable == null) continue;
          
          double totalObtained = 0.0;
          final innerTds = innerTable.querySelectorAll('td');
          for (final markTd in innerTds) {
            final strongText = markTd.querySelector('strong')?.text.trim() ?? '';
            if (strongText.contains('/')) {
    ...
    final ysList = [
      '2026_27', '2025_26', '2024_25', '2023_24', '2022_23'
    ];
    for (final ys in ysList) {
      final ttHtml = await _fetchPage('https://academia.srmist.edu.in/srm_university/academia-academic-services/page/My_Time_Table_$ys');
      if (ttHtml != null) {
    ...
    return {
      ...
      'marks': {
        ...
        'raw_tables': [], // We don't have end-sem marks natively via My_Attendance
        'semester_results': [],
        'attendance_details': []
      }
    };
